...
public class EmployerAccessLogic {
    ...
    public List<Employer.AllissuesEmployer> GetCompletedAllissuesEmployer(String employee_ID) {
        try {

            List<Employer.AllissuesEmployer> AllissuesEmployer = new ArrayList<Employer.AllissuesEmployer>();

            String query = " SELECT * FROM notcomplitedallissues where Employee_ID = '" + employee_ID + "' AND Status ='Completed'";
            ResultSet result = singleConn.ExecuteQueryResultSet(query);
    ...
    public List<Employer.AllissuesEmployer> GetCompletedAllissuesAdmin(String searchText) {
        try {
    ...
    public boolean updatmyAllissuesEmployerTODB(Employer.myissuesEmployer objMyissues) {
        ...
        try {
            ...
            // String query1=   "UPDATE notcomplitedallissues  set comment ='" +objMyissues.getcomment() +"' where ncallissuesid ='"+46+"';";
            //type, name, id, email, password1, phone, password2, address
            boolean result = singleConn.ExecuteQuery(query);
            return result;
        } catch (Exception ex) {
    ...
    public boolean submitmyAllissuesEmployerTODB(Employer.myissuesEmployer objMyissues) {

        try {

            String query = "update notcomplitedallissues  set comment = '" + objMyissues.getcomment() + "',Status = 'Completed' where ncallissuesid = '" + objMyissues.getissueid1() + "';";
            // String query1=   "UPDATE notcomplitedallissues  set comment ='" +objMyissues.getcomment() +"' where ncallissuesid ='"+46+"';";
            //type, name, id, email, password1, phone, password2, address
            boolean result = singleConn.ExecuteQuery(query);
            return result;
    ...
    public List<Employer.myissuesEmployer> GetmyissuesEmployer(String employee_ID) {
        try {
            ...
            String query = " SELECT  ncallissuesid,issuetype,inquiry_type,stname,itnumber,email,phoneNo,degree_type,degreename,registerdyear,intake,year,"
                    + "semester,batch,subjectname,exam,date,time,comment,allexinquirydetails,Employee_ID FROM notcomplitedallissues where Employee_ID = '" + employee_ID + "' AND Status ='Not-Completed'";
            ResultSet result = singleConn.ExecuteQueryResultSet(query);
